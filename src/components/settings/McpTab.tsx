 ...
 *
 * 🔴 红线：服务默认只监听 127.0.0.1（局域网直连需单独开启，凭令牌接入）；
 *   每个请求要带令牌；删除只进回收站；写入全部计入调用记录；
 *   八项写权限默认全开但可逐项关。
 */
    ...
    async (next: boolean) => {
      if (next) {
        const ok = await confirmDialog({
          title: "开启知识库 MCP 服务？",
          message:
            "开启后，本机任何能拿到访问令牌的程序都可以读取、修改和删除你的笔记。服务默认只监听 127.0.0.1；若再打开局域网访问，局域网内拿到令牌的机器也能连接。删除只进回收站，随时可恢复；每次写入都会计入调用记录。开启后可以在面板里逐项关掉写权限。",
          confirmText: "开启服务",
          variant: "warning",
        });
        if (!ok) return;
  ...
  return (
    <>
      ...
      <ToggleRow
        ...
        hue="sync"
        label="知识库 MCP 服务"
        desc="让 Claude Code 等 AI 工具读写你的笔记（默认仅本机，可选局域网凭令牌接入）"
        value={mcp.status.running}
        tooltip="在本机开一个 MCP 服务，AI 工具凭令牌搜索、读取与修改笔记；可选允许局域网机器凭令牌接入"
        ...
        detail={
          <>
            ...
            <p>📌 默认只监听 <b>127.0.0.1 回环地址</b>；可选开启<b>局域网访问</b>（凭令牌接入）</p>
