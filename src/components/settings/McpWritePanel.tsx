 ...
 *
 * 八个开关按风险分四组：新增 / 修改 / 删除与恢复 / 整理文件夹。
 * 分组由**后端的返回顺序**决定（`WriteKind::ALL` 就是按风险递增排的），
 * 前端只按下面 `GROUP_AT` 里那几个分组点切开——不重新排序，
 * 否则两边各有一套风险判断。
 *
...
const GROUP_AT: Record<string, string> = {
  ...
  // 🔴 这一行不是可选的。新档 `Structure` 在 `WriteKind::ALL` 里排在
  // `Restore` 之后，不给它分组点就会落到「删除与恢复」那个小标题下面
  // ——而整理文件夹既不是删除也不是恢复。分组靠后端顺序（见上），
  // 所以漏了不会报错，只是归错类。
  mcp_write_structure: "整理文件夹——改的是你的目录结构，笔记一篇不删",
};
