 ...
 * 待沉淀「入选原因」的**唯一**展示表。
 *
 * 后端 `kb_inbox.rs::reason_expr()` 只回原始值（`star`/`research`），
...
> = {
  ...
  // 「搜回过」而不是「找回」：后者是我们内部的叫法，用户不知道它指
  // 「搜出来后真的用了那条」。具体次数在 `signalText` 里说清。
  research: { label: "搜回过", only: "只看搜回过的", Icon: Search, badge: "badgeHit" },
};

/** 按优先级排好的全部原因。顺序与后端 `reason_expr()` 的 `WHEN` 一致。 */
export const REASON_ORDER: InboxReason[] = ["star", "research"];
