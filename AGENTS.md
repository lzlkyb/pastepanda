...
2. **改之后**跑 `npm run lint:ui` —— 它把文档里**可判定**的条文变成机器检查，**只查本次改动的行**（存量不拦，见文档 §10「不搞大扫除」）。
3. 机器查不到的那些（U1 / U3 四状态 / U4 / L1 / L4 / L5 / V4 / V5 + 黑话测试）逐条过文档 §9；`npm run lint:ui:all` 跑完会把这 13 条打出来提醒。

**为什么第 2 步不能省**：2026-09-22 审远程电脑工作台抓到的偏离（11px 灰字用在说明文字而非 meta、`--section-bg` 与 `--app-bg` 差值过小导致材质分层失效、`--card-selected-bg/shadow` 定义了却没用）**都不是「不知道规则」，是「改的时候没对着规则查」**。文档是判据，脚本是执行链。

| 命令 | 用途 |
|---|---|
| `npm run lint:ui` | 查本次改动的行（默认）|
| `npm run lint:ui:all` | 全库基线报告，**不拦**（加 `--strict` 才当门禁）|
| `npm run lint:ui:selftest` | 校验器自己的守卫测试（漏报 / 误报 / 豁免 / 计数）|
...
**豁免**：确是「间距和背景色都不行」这类情况，就在那一行写 `/* ui-rule-ok: <理由> */`——
**必须带理由**，只写标记不写理由会被当成违规报出来（理由留给下一个读这段代码的人）。
