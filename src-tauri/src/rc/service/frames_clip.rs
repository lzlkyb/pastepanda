...
impl RcService {
    ...
    ///
    /// C5（2026-09-23 复审，🔴 P3-17 于 2026-09-25 兑现）：超时/作废**不再是
    /// `Ok(None)`，而是 `Err`**——`None` 唯一诚实的含义是「回包说了、对方
    /// 剪贴板为空」，前端已把空串当「对方是空的」；把链路问题折叠成空值
    /// 就是撒谎（规则 15.3）。此前函数头这么说、代码却在 deadline 耗尽时
    /// 返回 `Ok(None)`，契约全靠前端旧分支兜着；现在以代码为准。
    ...
    pub async fn pull_clipboard(&self) -> Result<Option<String>, String> {
        ...
        // 🔴 P3-17（2026-09-25 审计）：deadline 耗尽 = 链路问题，改 Err
        //（秒数从常量推导，别把「4 秒」写死两处）。
        Err(format!(
            "剪贴板拉取超时：对方 {} 秒未回包",
            CLIPBOARD_PULL_TIMEOUT_MS / 1000
        ))
    }
