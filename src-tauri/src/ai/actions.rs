...
pub const ACTIONS: &[AiAction] = &[
    ...
    // 每日整理（H3 行为层）。与周报同一类：喂进去的只有数字与来源名。
    // max_tokens 比周报低：一天的事本来就比一周少，而且要的是 3~4 行而不是一篇。
    AiAction {
        id: "ai-daily-brief",
        label: "生成今日小结",
        description: "把今天的分段与统计翻译成一段小结",
        icon: "calendar-days",
        max_tokens: 800,
        options: &[],
        content_types: &[],
...
pub fn build_prompt(
    ...
    let user = match action.id {
        ...
        // 每日整理。两句约束必须留着：
        // ①「只有应用名没有具体内容」——告诉模型它看不到内容，否则它会顺着应用名编「你在改 XX bug」；
        // ②「不要猜具体在做什么项目」——那是内容层（C3，需授权读原文）才能回答的事。
        "ai-daily-brief" => format!(
            "下面是我一天使用剪贴板的行为统计，按时间分成了几段。\
             里面**只有应用名与数字，没有任何具体内容**。\
             用中文写 3~4 行，说说这一天的重心落在哪里、节奏是怎样的。\
             **不要猜我具体在做什么项目或任务**，也不要编统计里不存在的细节：\n\n{}",
            trimmed
        ),
