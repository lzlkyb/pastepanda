...
mod tests {
    ...
    /// 状态窗与选区相交 = 会被 BitBlt 拼进长图，这是四角候选位唯一不能出错的地方。
    /// ⚠️ 例外：兜底位置（选区几乎占满屏幕、四角全被占时退到屏幕底部居中）允许相交，
    /// 因为 capture_region 在截屏瞬间会临时隐藏状态窗（hide + 等 DWM 重合成），
    /// 不会被拼进长图；留窗是为了长截图期间保留可见的「停止/放弃」入口。
    /// 只要四角候选位放得下，就绝不允许相交（下面的用例都在验证这一点）。
    ...
    #[test]
    fn 选区占满屏幕时兜底到屏幕底部居中() {
        // 四角全被占、无处可放时，兜底退到屏幕底部居中 (810, 1016)。
    ...
    #[test]
    fn 只剩中间一条缝时也兜底给位置() {
        // 上下各占一半多一点，四角全被占，中间缝隙放不下 48 高 ——
        // 无不相交位置，与上例同理兜底到屏幕底部居中（相交由截屏隐藏兜底）。
        assert_eq!(pick((0, 0, 1920, 1060)), Some((810, 1016)));
    }
