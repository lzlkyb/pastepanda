...
// ===== O-4 切片层 =====
//
// 验收口径（原出 `docs/O-4-切片层定案.md` §7，该文档已于 2026-09-08 并入
// `docs/PastePanda-规划与排期.md` 并删除，所以把五条直接抄在这里。
// 那五条不是随便列的：它们共同保证了「重新切一遍不用全量重算向量」，
// 而那正是 O-4 存在的理由）：
//
//   ① 同一篇连算两次，切片 ID 与 `content_hash` 完全一致（确定性）
//   ② 改笔记里一个字，**只有一个**切片的 hash 变
//   ③ 空节不产生切片
//   ④ 超长节切出来的每个切片都 ≤ 窗口，且能拼回原文（无丢失）
//   ⑤ 每个切片都能反查到它的节标题路径
//
// ❗ 窗口/重叠的具体数值（先用 512/64）**未定**，等 AM-5 复核。
