...
//!
//! 对应知识库主规划 §8.1 2️⃣（建表）与 §7 L1（检索第三路）。
//! ❗ 那份文档已于 2026-09-08 并入 `docs/PastePanda-规划与排期.md`，原文见 git 历史。
...
///
/// # 那四处「故意不刷」现在只剩两处（2026-09-07 更新）
///
/// 原文列的是：`pinned` 切换 / `summary` 写入 / `folder_id` 移动 / `deleted_at` 软删，
/// 理由是「它们连 `updated_at` 都不刷，而界面最近修改按 `updated_at` 排，
/// 把置顶算成一次修改会让列表乱跳」。
///
/// 🔴 **那个顾虑是对的，但它只管得着 `updated_at`。**
/// M6-P2 已经把同步的行版本号迁成了**另一个列** `updated_ms`，
/// 两者独立——可以只刷 `updated_ms` 而不动 `updated_at`，
/// 同步看得见这一行变过，而界面排序不受影响。那句「等 M6 主体决定是否拆出
/// 独立的行版本号」描述的决定，P2 已经做完了。
///
/// 现状：
/// - `deleted_at` 软删 → 归 M6-P4 墓碑，不走这条路。
/// - `folder_id` 移动 → ✅ **已刷 `updated_ms`**（见 `note_set_folder`）。
///   ❗ 光刷这一处不够，另有两处缺一不可，理由写在那个函数的注释里。
/// - `summary` 写入 → **故意不刷**，而且这里已经是最优解，不用再看：
///   ❗ 它还是会同步的——`summary` 在 frontmatter 里（`note_md.rs`），
///   `note_import_dir` 也会写回去。所以真实行为是
///   「**笔记因为别的原因同步时，摘要顺路过去；摘要自己不制造流量**」。
///   刷戳只会让 AI 批量生成摘要变成一次同步风暴，而换不来任何东西。
/// - `pinned` 切换 → **仍不刷**，但阻塞点不是这里：前置字段里
///   根本没有 `pinned`（只有 title/tags/created/updated/summary），
///   而文件夹靠目录层级表达。要同步置顶得先给它一个线上载体。
///
///   🔴 **触发条件（写成可判定的，不写「等某人决定」）**：
///   出现**第二个**「只在库里、不在 markdown 里」的字段时再动。
///   那时❗ **不要往 frontmatter 塞第二个**：清单已经带着每篇的元数据
///   （`id \t ms \t rel`），加一份「同步专用字段」的旁挂文件，
///   N 个字段一次性解决，且完全不动 vault 导出格式与 Obsidian 互操作性。
///   往 frontmatter 塞了第二个，格式就回不来了——那才是不可逆的那一步。
///
///   ❗ 并且它不是「3 行」：和文件夹一样是**同一形状的三处改动**
...
impl DataStore {
    ...
    /// 本机当前的 HLC 下界。**≥ 库里任何一个时间戳**（见 `DataStore::new` 里的算法）。
    ///
    /// 同步会话开场互相报这个值，两边取大就是这一轮的**共同高水位**，
    /// 而它正是「这一轮之后游标该推到哪儿」。参见 [`crate::sync::session`]。
    pub fn sync_high_water_ms(&self) -> i64 {
        self.hlc.floor()
    }
