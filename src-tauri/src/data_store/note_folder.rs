...
/// 深度上限（顶层计为 1）。
///
/// 这个数字**不是数据模型或性能的要求，纯类是侧栏的宽度预算**：
/// 180px 栏宽减去缩进、折叠三角、计数胶囊之后，文件夹名字还剩多少地方。
///
/// 3 → 4（用户拍板）。能抬的依据是缩进从 13px 降到了 10px，而且文件夹名
/// 现在有 `title` 悬停可看全（`FolderTree.tsx`）。代价是第 4 层的名字区
/// 只剩 ~62px（约 4.7 个中文字），**基本必截断**——这是明知的取舍，
/// 不是没算过。再往上加就得先把侧栏加宽或改成横向可滚。
///
/// ❗ 改它会连带影响：建/移文件夹的校验（本文件）、**vault 导入的平接点**
///   （`note_vault.rs::ensure_folder_path`）、侧栏的缩进宽度账
///   （`FolderTree.module.css` 文件头）。这三处必须一起看。
pub const MAX_FOLDER_DEPTH: i64 = 4;
