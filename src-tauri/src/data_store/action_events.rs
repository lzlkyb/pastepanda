...
/// 卡片使用信号的哨兵动作 id（不是真实变换动作，聚合权重时排除）。
///
/// ❗ 名字叫 `paste`，但 2026-09-08 起它同时承载**复制**事件
/// （`outcome = "copied"`）——剪贴板工具里复制才是主要的「用了它」，
/// 而它一直没被记过。没新开一个 `copy` 哨兵是因为排除逻辑散在
/// **六处**查询里（`action_id <> ?`），漏一处不报错、只静默污染权重。
/// 哨兵的真实含义是「卡片使用事件」，复制/粘贴的区别交给 `outcome`。
///
/// 🔴 `VALUE_PRESERVE_SQL`（豁免过期清理）仍只认 `outcome='pasted'`，
/// 复制不会因此拿到免死金牌——这是有意保守的。
