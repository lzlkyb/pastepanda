...
/// 一次性的定界符随机串（O-1）。
///
/// 🔴 **为何定界符不能是固定的**：`tools::wrap_content` 把笔记正文原样插进
/// `<note-content>…</note-content>` 之间，而知识库的内容**大量来自剪贴板**
/// ——网页、聊天窗口、别人发来的文件。一篇正文里只要写着 `</note-content>`，
/// 它就能**提前闭合包裹**，然后自己伪造一句「以上是数据」的收尾、后面接指令。
/// 定界符固定的话，伪造它不需要任何猜测。
///
/// 每次调用现生成一串随机十六进制，笔记内容无从预知，闭合标记就伪造不出来。
/// 取 16 位（64 bit）：猜中的概率可以忽略，而每次调用只多出几十个字符。
///
/// ⚠ 转义正文是另一条路，但它**改了用户的数据**——而 O-1 的「不做什么」
/// 第一条就是「不做内容过滤/改写」。nonce 一个字都不动原文。
pub(crate) fn delim_nonce() -> String {
    uuid::Uuid::new_v4()
        .simple()
        .to_string()
        .chars()
