...
/// 空闲时的**本地**脏检查间隔（秒）。
///
/// 只问一句 [`crate::data_store::DataStore::has_changes_since`]，不建任何连接。
/// 它决定的是「本机刚改的东西最多多久被发现」的**上限**——
/// 正常情况下写入信号（方案 B）会先把循环叫醒，根本等不到这个间隔。
pub const IDLE_CHECK_SECS: u64 = 5;

/// 什么都没变时的**兜底**同步间隔（秒）。
///
/// 🔴 不能去掉。「脏了才拨」只能盖住「本机有改动」，而对端的改动是靠
/// **它自己拨过来**。若它改完拨不通（本机当时离线）、退避到休眠不再试，
/// 而它又是 WAN 对端（没有组播，`Heard::Fresh { returned }` 那条叫醒用不上），
/// 就只剩这个心跳能把它捣回来。
pub const HEARTBEAT_SECS: u64 = 600;

/// 两次会话之间的最小间隔（秒）。
///
/// ❗ 去抖用。连续敲键盘时每一次写入都会触发写入信号，
/// 没有这个下限就会变成“边打字边拨号”。
pub const MIN_SESSION_GAP_SECS: u64 = 10;
