...
> 本文件由 `scripts/gen-design-index.mjs` 自动生成（最近生成：2026-09-22）。**不要手改**——下次生成会覆盖。要调分组规则，改脚本里的 `RULES`。
...
_共 208 份设计稿。重新生成：`npm run gen:design-index`_
